...
public class RewardServiceTest {
	...
	public void testRewardService_portfolioIsNull_fail() {
		try {
			...
			Assert.assertEquals(ex.getMessage(), "Portfolio not found");
			Assert.assertEquals(ex.getResultCode().getResultCode(),
					Integer.valueOf(2));
		}
	}
	...
	public void testRewardService_eligibilityIsNull_fail() {
		try {
			...
			when(mock.getAccountElgibiltyByAccountId(anyInt()))
					.thenReturn(null);
			Portfolio portfolio = new Portfolio();
			portfolio.setChannelType(ChannelType.KIDS);
			...
			Assert.assertEquals(ex.getMessage(), "Eligible type not found");
			Assert.assertEquals(ex.getResultCode().getResultCode(),
					Integer.valueOf(4));
		}
	}
	...
	public void testRewardService_accountNotValid() {
		try {
			when(eligibilityService.getAccountElgibiltyByAccountId(anyInt()))
					.thenThrow(
							new SkyServiceException(
									ResultCodeType.INVALID_ACCOUNT));
			Portfolio portfolio = new Portfolio();
			portfolio.setChannelType(ChannelType.KIDS);
			rewardService.getRewards(1, portfolio);
			Assert.fail();
	...
	public void testRewardService_internalError_fail() {
		...
		when(eligibilityService.getAccountElgibiltyByAccountId(anyInt()))
				.thenThrow(
						new SkyServiceException(ResultCodeType.INTERNAL_ERROR));
		Portfolio portfolio = new Portfolio();
		portfolio.setChannelType(ChannelType.KIDS);
		rewardService.getRewards(1, portfolio);
		Assert.fail();
